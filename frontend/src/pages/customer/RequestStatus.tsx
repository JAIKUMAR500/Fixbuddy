...
import { ArrowLeft, MapPin, Clock, Phone, MessageSquare, CheckCircle, Circle, Play, Flag } from "lucide-react";
        ...
        {request && (
          <>
                  ...
                  {provider ? (
                    ...
                    <p className="text-sm text-slate-500 mt-1">
                      {waitingForAccept && invitedCount
                        ? `${invitedCount} worker${invitedCount === 1 ? "" : "s"} asked. The first one to accept takes this job.`
            ...
            {waitingForAccept && !worker && (
              <Button size="lg" fullWidth onClick={() => navigate("matched-providers")}>
                Request more workers
              </Button>
            )}

            {worker && request.providerId === user?.id && ["accepted", "scheduled", "in_progress"].includes(request.status) && (
              <Card padding="lg">
                <JobProgress status={request.status} />
                <div className="mt-5">
                  {jobPrimaryAction(request.status) === "start" && (
                    <Button size="lg" fullWidth loading={acting === "start"} onClick={async () => {
                      if (!activeRequestId) return;
                      setActing("start");
                  ...
                  {jobPrimaryAction(request.status) === "complete" && (
                    <Button size="lg" fullWidth loading={acting === "complete"} onClick={async () => {
                      if (!activeRequestId) return;
                      setActing("complete");
