...
export default function WorkerPassportPage() {
  ...
  React.useEffect(() => {
    if (!passport) return;
    setBio(passport.bio || "");
    setExperienceYears(String(passport.experience || ""));
    setSkills(passport.skills.map((skill) => skill.name).join(", "));
  }, [passport]);
  ...
  return (
    <div className="p-4 lg:p-6 pb-24 lg:pb-6 max-w-3xl space-y-5">
      ...
      {passport && <>
        <Card padding="lg" className="bg-navy text-white"><div className="flex items-start justify-between gap-4"><div><p className="text-xs uppercase tracking-wider text-slate-300">FixBuddy Professional</p><h2 className="mt-2 text-2xl font-bold">{passport.name}</h2><p className="mt-1 text-slate-300">{passport.skills[0]?.name || passport.category || "Service professional"}</p></div><UserRound className="w-10 h-10 text-sky-300" /></div><div className="grid grid-cols-3 gap-3 mt-6 text-center"><div><p className="text-2xl font-black">{passport.completedJobs}</p><p className="text-xs text-slate-300">Completed</p></div><div><p className="text-2xl font-black">{passport.ratingAvg.toFixed(1)}</p><p className="text-xs text-slate-300">Rating</p></div><div><p className="text-2xl font-black">{passport.verified ? "Yes" : "Pending"}</p><p className="text-xs text-slate-300">Verified</p></div></div></Card>
        ...
        <Card padding="md"><h2 className="font-semibold text-slate-900 mb-3">Verified skills</h2><div className="flex flex-wrap gap-2">{passport.skills.map((skill) => <span key={skill.name} className="inline-flex items-center gap-1 rounded-full bg-emerald-50 px-3 py-1.5 text-sm text-emerald-800"><Plus className="w-3 h-3" />{skill.name}{skill.verified ? " · Verified" : " · Pending"}</span>)}</div><div className="mt-4 flex flex-wrap gap-2">{(passport.badges || []).map((badge) => <span key={badge.id} className="rounded-full bg-amber-50 px-3 py-1.5 text-sm font-semibold text-amber-800">{badge.icon} {badge.label}</span>)}</div>{passport.publicProfileUrl && <img className="mt-4 h-28 w-28 rounded-xl border border-slate-200" alt="Public passport QR" src={`https://quickchart.io/qr?size=180&text=${encodeURIComponent(`${window.location.origin}${passport.publicProfileUrl}`)}`} />}</Card>
      </>}
    </div>
  );
}
