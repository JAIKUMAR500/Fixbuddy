...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    await notify(taken.customerId, {
      type: "success",
      text: `${req.user.provider?.businessName || req.user.name} accepted your job. Open your active job to track them.`,
      requestId: taken._id,
    });
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const cancelledBy =
      isAdmin(req.user.role) && String(doc.customerId) !== req.userId && String(doc.providerId) !== req.userId
        ? "admin"
        : String(doc.customerId) === req.userId
          ? "customer"
          : isSeeker(req.user.role)
            ? "worker"
            : "business";
    const traveling = ["on_the_way", "arrived", "otp_verified", "in_progress"].includes(statusWas);
    let compensation = 0;
    let eligible = false;
    if (cancelledBy === "customer" && doc.providerId && traveling) {
      const { getSettings } = await import("../models/PlatformSettings.js");
      const settings = await getSettings();
      compensation = Number(settings.travelCompensationInr || 75);
      eligible = compensation > 0;
    }
    doc.cancelReason = reason;
    doc.travelCompensation = compensation;
    doc.cancelledAt = new Date();
    doc.cancelledBy = cancelledBy;
    pushTimeline(doc, "cancelled", reason);
    await doc.save();
    const { CancellationRecord } = await import("../models/CancellationRecord.js");
    await CancellationRecord.create({
      requestId: doc._id,
      workerId: doc.providerId,
    ...
    if (eligible && doc.providerId) {
      ...
      await Transaction.create({
        ...
        kind: "payout",
        status: "paid",
        note: "Travel / cancellation compensation",
      });
      await User.updateOne({ _id: doc.providerId }, { $inc: { walletBalance: compensation } });
      await notify(doc.providerId, {
        type: "success",
        text: `Customer cancelled after you started travelling. Travel compensation ₹${compensation} was added to your wallet.`,
        requestId: doc._id,
      });
    } else {
